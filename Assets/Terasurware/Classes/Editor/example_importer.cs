...
public class example_importer : AssetPostprocessor {
	private static readonly string filePath = "Assets/example.xlsx";
	private static readonly string exportPath = "Assets/example.asset";
		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(0); p.question = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(1); p.hp = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(2); p.q = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(3); p.qans = (cell == null ? "" : cell.StringCellValue);
						s.list.Add (p);
					}
					data.sheets.Add(s);
				}
